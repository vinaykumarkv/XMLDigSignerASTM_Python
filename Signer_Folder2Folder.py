...
# Configure logging
logging.basicConfig(filename='signing_errors.log', level=logging.ERROR,
...
def sign_files_in_directory(source_folder, destination_folder):
    private_key_pem, public_key_pem = generate_keys()
    key = load_private_key(private_key_pem)

    for filename in os.listdir(source_folder):
        if filename.endswith(".xml"):
            input_xml_file = os.path.join(source_folder, filename)
            output_signed_xml_file = os.path.join(destination_folder, f"signed_{filename}")
            try:
                if not is_xml_signed(input_xml_file):
                    sign_xml(input_xml_file, output_signed_xml_file, key, public_key_pem)
                    print(f"Signed XML saved to: {output_signed_xml_file}")
            except Exception as e:
                logging.error(f"Failed to sign {input_xml_file}: {e}")
...
if __name__ == "__main__":
    ...
    sign_files_in_directory(source_folder, destination_folder)
